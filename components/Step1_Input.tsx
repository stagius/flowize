...
export const Step1_Input: React.FC<Props> = ({ onTasksGenerated, existingTasks, model, geminiApiKey }) => {
  ...
  return (
    <div className={`grid grid-cols-1 md:grid-cols-2 ${SPACING.sectionGap} h-full max-h-none md:max-h-[calc(100vh-12rem)] overflow-y-auto md:overflow-hidden`}>
      {/* Input Section */}
      <div className="bg-white dark:bg-slate-900/50 backdrop-blur-sm p-4 md:p-6 rounded-2xl border border-slate-200 dark:border-slate-800 flex flex-col relative group h-auto md:h-full min-h-[300px] md:min-h-[460px]">
        ...
        <div className="mb-3 md:mb-4">
          <h2 className={`${TYPOGRAPHY.sectionTitleClass} flex items-center gap-2 text-base md:text-lg`}>
            <AlignLeft className="w-5 h-5 text-cyan-700 dark:text-cyan-400" />
        ...
        <textarea
          className="flex-1 w-full min-h-[160px] md:min-h-[280px] p-3 md:p-4 bg-slate-100 dark:bg-slate-950/50 border border-slate-300 dark:border-slate-800 rounded-xl focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500/50 outline-none resize-none text-slate-900 dark:text-slate-300 font-mono text-sm placeholder:text-slate-600 dark:placeholder:text-slate-700 transition-all shadow-inner"
        ...
        <div className="mt-3 md:mt-4 flex justify-end">
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 md:px-4 md:py-2 text-xs md:text-sm rounded-lg border border-cyan-600/40 dark:border-cyan-500/30 bg-cyan-500/25 dark:bg-cyan-500/15 text-cyan-900 dark:text-cyan-100 hover:bg-cyan-500/35 dark:hover:bg-cyan-500/25 hover:border-cyan-600/50 dark:hover:border-cyan-400/40 disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-medium"
          >
            {isAnalyzing ? (
              <>
              ...
              <>
                <Sparkles className="w-4 h-4 text-yellow-600 dark:text-yellow-300" />
                Analyze & Format
              </>
            )}
          </button>
      ...
      {/* Preview Section */}
      <div className="bg-white dark:bg-slate-900/50 backdrop-blur-sm p-4 md:p-6 rounded-2xl border border-slate-200 dark:border-slate-800 flex flex-col h-auto md:h-full min-h-[250px] md:min-h-[460px] overflow-hidden">
          ...
          <div className="flex items-center gap-2 md:gap-3">
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 text-xs font-medium rounded-lg border transition-all ${showNewOnly
                  ? 'bg-cyan-500/30 dark:bg-cyan-500/20 border-cyan-600/60 dark:border-cyan-500/50 text-cyan-900 dark:text-cyan-300'
            ...
            >
              <span className={`w-3.5 h-3.5 rounded border flex items-center justify-center transition-all ${showNewOnly ? 'bg-cyan-600 dark:bg-cyan-500 border-cyan-600 dark:border-cyan-500' : 'border-slate-400 dark:border-slate-500'
                }`}>
            ...
            <span className="text-xs font-semibold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 px-2 py-1 rounded-full border border-slate-300 dark:border-slate-700">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto space-y-2 md:space-y-3 pr-2 custom-scrollbar">
          {isAnalyzing ? (
            ...
            <div className="h-full flex flex-col items-center justify-center text-slate-600 dark:text-slate-700 opacity-50">
              <div className="w-16 h-16 mb-4 border-2 border-dashed border-slate-300 dark:border-slate-800 rounded-xl flex items-center justify-center">
                ...
                return (
                  <div key={task.id} className={`mt-4 p-3 md:p-4 border rounded-xl transition-all group relative ${isNew ? 'bg-cyan-100/50 dark:bg-cyan-950/30 border-cyan-500/60 dark:border-cyan-500/40 hover:border-cyan-600/80 dark:hover:border-cyan-400/60 hover:bg-cyan-100/70 dark:hover:bg-cyan-900/40' : 'bg-slate-50 dark:bg-slate-900/80 border-slate-300 dark:border-slate-800 hover:border-slate-400 dark:hover:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800'}`}>
                    ...
                    <div className="flex items-start justify-between gap-2 md:gap-4">
                      <span className={`font-medium text-xs md:text-sm truncate ${isNew ? 'text-cyan-900 dark:text-cyan-100' : 'text-slate-900 dark:text-slate-200'}`}>{task.title}</span>
                    ...
                    <p className="text-xs text-slate-600 dark:text-slate-400 line-clamp-2 mt-1 md:mt-1.5 leading-relaxed">{task.description}</p>
                    <div className="mt-2 md:mt-3 flex gap-2 items-center">
                      <span className={`text-[10px] px-2 py-0.5 rounded border ${isNew ? 'bg-cyan-500/30 dark:bg-cyan-500/20 text-cyan-900 dark:text-cyan-300 border-cyan-600/40 dark:border-cyan-500/30' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-300 dark:border-slate-700'}`}>{task.group}</span>
                      {isNew && <span className="text-[10px] text-cyan-800 dark:text-cyan-400 font-medium">New</span>}
                    </div>
                  </div>
                );
              })
