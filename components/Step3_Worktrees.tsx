            ...
            {activeAgentConsoleSlotId !== null && (
                <div className="absolute inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm bg-slate-950/60 animate-in fade-in duration-200">
                    <div className="w-full max-w-3xl bg-slate-950 border border-slate-800 rounded-xl shadow-2xl flex flex-col overflow-hidden ring-1 ring-slate-800 h-[600px]">
                                ...
                                {activeAgentTask && activeAgentCommand && (
                                    ...
                                    >
                                        <span className="inline-flex items-center gap-1.5">
                                            {copiedAgentCommandTaskId === activeAgentTask.id ? <Check className="w-3.5 h-3.5 text-emerald-300" /> : <Copy className="w-3.5 h-3.5" />}
                        ...
                        <div className="px-4 py-2 border-b border-slate-800 bg-slate-900/60 text-[11px] text-slate-400 font-mono">
                            {runningAgentTaskId && activeAgentTask?.id === runningAgentTaskId
                                ? 'Agent status: running...'
                                : `Agent status: ${activeAgentTask?.agentRunState || 'idle'}`}
                        </div>

                        <div className="flex-1 bg-black/50 p-4 font-mono text-xs overflow-y-auto custom-scrollbar whitespace-pre-wrap text-slate-300">
                            {runningAgentTaskId && activeAgentTask?.id === runningAgentTaskId
                        ...
                        <div key={slot.id} className={`rounded-xl border flex flex-col md:flex-row overflow-hidden transition-all relative flex-shrink-0 min-h-[250px] ${theme.border} ${theme.bg} ${assignedTask ? 'shadow-lg' : ''}`}>
                            ...
                            <div className="w-full md:w-56 bg-slate-950/50 border-b md:border-b-0 md:border-r border-slate-800 p-3 md:p-4 flex flex-row md:flex-col justify-between md:justify-center items-center text-left md:text-center flex-shrink-0 gap-3 md:gap-0">
                                ...
                                <div className="hidden md:flex flex-col items-center gap-2 w-full mt-2">
                                    ...
                                    >
                                        {cleaningSlot === slot.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Trash2 className="w-3 h-3" />}
                                        Cleanup
                                    </button>
                                </div>
                                ...
                                <div className="md:hidden flex flex-col items-end gap-2">
                                     <button
                                        ...
                                        title="Cleanup"
                                    >
                                        {cleaningSlot === slot.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Trash2 className="w-3.5 h-3.5" />}
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="flex-1 p-3 md:p-4 relative flex flex-col min-w-0">
                                {!assignedTask ? (
                                    ...
                                    <div className="flex flex-col items-center justify-center h-full gap-4 text-slate-500 py-6 md:py-8">
