...
export const Step5_Review: React.FC<Props> = ({ tasks, onApprovePR, onRequestChanges, onCheckStatus, bridgeHealth, settings }) => {
  ...
  return (
    <>
      <div className="mb-4">
        <div className="lg:col-span-2 bg-slate-100 dark:bg-slate-900/50 backdrop-blur-sm rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
          <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900/80 flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="flex gap-2">
              <div className={`rounded-lg border px-3 py-1.5 text-xs font-semibold ${hostReady
                ...
                {hostReady ? 'Review ready' : bridgeHealth?.status === 'checking' ? 'Checking host...' : 'Host unavailable'}
              </div>
              <button
                type="button"
