...
export const Step6_Merge: React.FC<Props> = ({ tasks, onMerge, onResolveConflict, onFetchMerged, settings }) => {
  ...
  return (
    <>
      {/* Ready to Merge */}
      <div className="bg-white dark:bg-slate-900/50 backdrop-blur-sm rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex-shrink-0 flex flex-col max-h-[45%] mb-4">
        <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-green-50 dark:bg-green-500/5 flex justify-between items-center flex-shrink-0">
                ...
                <div key={task.id} className="border border-green-300 dark:border-green-500/20 bg-green-50 dark:bg-green-500/5 rounded-xl p-5 flex flex-col justify-between min-h-[12rem] h-auto hover:bg-green-100 dark:hover:bg-green-500/10 transition-colors relative overflow-hidden group">
                  ...
                  <div>
                    <div className="flex justify-between items-start mb-2">
                      <span className="text-xs font-mono text-green-700 dark:text-green-300/80">
                        {buildPrUrl(task) ? (
                          <a
                            href={buildPrUrl(task) as string}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="hover:text-green-800 dark:hover:text-green-200 hover:underline inline-flex items-center gap-1"
                          >
                      ...
                      <div className={`flex items-center gap-1 text-[10px] border px-1.5 py-0.5 rounded shadow-sm ${task.vercelStatus === 'success'
                        ? 'bg-emerald-100 dark:bg-emerald-500/10 border-emerald-400 dark:border-emerald-500/20 text-emerald-700 dark:text-emerald-400'
                        : task.vercelStatus === 'failed'
                          ? 'bg-red-100 dark:bg-red-500/10 border-red-400 dark:border-red-500/20 text-red-700 dark:text-red-400'
                          : 'bg-yellow-100 dark:bg-yellow-500/10 border-yellow-400 dark:border-yellow-500/20 text-yellow-700 dark:text-yellow-400'
                        }`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${task.vercelStatus === 'success' ? 'bg-emerald-500' :
                    ...
                    <h4 className="font-bold text-slate-900 dark:text-slate-100 mb-1 line-clamp-2">{task.title}</h4>
                    <p className="text-xs text-slate-600 dark:text-slate-400 line-clamp-2 mb-3">{task.description}</p>
                    {task.mergeConflict && (
                      <div className="mb-3 inline-flex items-center gap-1 rounded-md border border-orange-400 dark:border-orange-500/30 bg-orange-100 dark:bg-orange-500/10 px-2 py-1 text-[10px] font-semibold text-orange-800 dark:text-orange-200">
                  ...
                  <button
                    ...
                    className={`w-full py-2.5 rounded-lg text-sm font-semibold flex items-center justify-center gap-2 transition-all ${mergingId === task.id || resolvingId === task.id
                      ? 'bg-slate-200 dark:bg-slate-800 text-slate-500 dark:text-slate-400 cursor-not-allowed'
                      : task.mergeConflict
                        ? 'bg-orange-600 hover:bg-orange-500 text-white shadow-lg shadow-orange-500/30 dark:shadow-orange-900/30'
                        : 'bg-green-600 hover:bg-green-500 text-white shadow-lg shadow-green-500/30 dark:shadow-green-900/30'
                      }`}
                  >
                    {mergingId === task.id || resolvingId === task.id ? (
                      ...
                      <>
                        {task.mergeConflict ? (
                          <AlertTriangle className="w-3.5 h-3.5 text-orange-100 dark:text-orange-200" />
                        ) : (
                          task.vercelStatus !== 'success' && <AlertTriangle className="w-3.5 h-3.5 text-yellow-200 dark:text-yellow-300" />
                        )}
      ...
      <div className="h-full min-h-0 flex flex-col gap-6">
        ...
        {/* Merged History */}
        <div className="flex-1 min-h-0 bg-white dark:bg-slate-900/50 backdrop-blur-sm rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col max-h-[85vh]">
          <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900/80 flex justify-between items-center">
            <h3 className="font-bold text-slate-600 dark:text-slate-300 flex items-center gap-2">
              <CheckCircle className="w-5 h-5 text-slate-600 dark:text-slate-400" /> Merged History
            </h3>
            <button
              onClick={handleFetch}
              disabled={isFetching}
              className="bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 px-3 py-1.5 rounded-lg text-xs font-medium transition-all flex items-center justify-center gap-2 border border-slate-300 dark:border-slate-700 shadow-sm"
            >
